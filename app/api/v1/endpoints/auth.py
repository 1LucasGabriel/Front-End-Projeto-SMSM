...
def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    if user_data.professional_registration and db.query(User).filter(User.professional_registration == user_data.professional_registration).first():
        raise HTTPException(status_code=400, detail="Registro profissional já registrado.")
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="Erro de integridade. Verifique se os dados são válidos.")
